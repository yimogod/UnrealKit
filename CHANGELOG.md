...
### Console Command Channel (Android sends over TCP)
- Console commands now go through a transport abstraction, `UnrealKit.Core.CommandChannel.ICommandTransport`, with two implementations: `HttpCommandTransport` (the existing Web Remote Control `PUT /remote/object/call`, serving Win64) and the new `TcpCommandTransport` (a single-line command plus single-line JSON response, serving Android). `AdbDeviceService.SendConsoleCommandAsync` now uses the TCP channel
- **Android could never reach Remote Control over HTTP.** The engine's `WebRemoteControl` module carries a `PlatformAllowList` of `Mac` / `Win64` / `Linux` only, so an Android build does not compile the HTTP server at all — every Android console command was connecting to a port nothing listens on. See `Doc/方案B-UE客户端控制台命令通道.md`
- **The UE-side TCP plugin is not part of this change.** Until it ships, Android console commands fail with `UKC101` (connection refused) — that is the expected state, not a misconfiguration
- Which platform uses which channel is stated in config, never inferred from the platform: new `ProjectSettings.AndroidCommandTransport` (default `Tcp`) and `Win64CommandTransport` (default `Http`), plus `CommandTcpPort` (default `39010`, also the `adb forward` port on both ends). A fork with a patched allowlist can set Android to `Http`, which a hardcoded branch would make impossible. An unrecognized value errors on project open instead of falling back — silently using HTTP would report a broken Android channel as "Remote Control not enabled", pointing the investigation the wrong way. An empty value means absent and falls back
- `adb forward` now forwards `ICommandTransport.Port` rather than a separately-read config value. With two sources, changing one forwarded to a port nothing was listening on
- New diagnostic code domain `UKC`: `UKC101` connect failed or timed out, `UKC102` command rejected by UE (`ok=false`, or a non-success HTTP status), `UKC103` protocol error (not single-line JSON, missing `ok`, truncated before the newline, response timeout). "The channel is down" and "the channel is up but this command was refused" are separate codes because the two lead to different fixes. A response missing `ok` is a protocol error, never a success — reading an unconfirmable response as success would show a command that never ran as having taken effect
- Commands containing a newline are rejected before connecting: the protocol frames on `\n`, so the peer would read one command as two and execute the tail as an unknown command. Response reading has a 1 MiB bound rather than relying on `StreamReader.ReadLineAsync`, which has no line-length limit — a peer that sends without ever sending a newline would otherwise grow the read without end
- **Breaking:** `AdbDeviceService` and `Win64DeviceService` take `CommandChannelOptions` + `ICommandTransport` in place of `RemoteControlOptions` + `IRemoteControlService`. `IRemoteControlService` / `RemoteControlService` / `RemoteControlOptions` are unchanged and still the HTTP client; the transport abstraction was added alongside rather than by renaming them, since `RemoteControl` genuinely names the HTTP scheme
- CLI and WPF are unchanged: `ConsoleCommandService` already depends on `IDeviceService`. No `--transport` option was added — the channel is a per-platform project setting, not a per-command choice
- Tests go from 330 to 351: 14 `TcpCommandTransportTests` against a local `TcpListener` fake covering success, `ok=false` with and without a reason, connection refused, non-JSON, missing `ok`, truncation, response timeout, and caller cancellation; plus device-service and project-config coverage for transport selection, forward-port sourcing, default channels per platform, and the config round trip
