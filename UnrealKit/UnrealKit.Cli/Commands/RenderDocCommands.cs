...
internal static class RenderDocCommands
{
    ...
    {
        CliOptions.EnsureOnly(options, CliOptions.Allowed("--script", "--args", "--output", "--workdir", "--format"));
    ...
    private static void WriteText(RenderDocExecutionResult result, string scriptPath)
    {
    ...
    private static int FailUsage()
    {
        ...
        Console.Error.WriteLine("  unrealkit renderdoc run --script <script.py> [--args <space-separated args>] [--output <dir>] [--workdir <dir>] [--format text|json]");
        return 2;
    }
}
