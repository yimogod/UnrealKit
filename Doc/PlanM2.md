...
最后更新：2026-08-09 | 框架 .NET 9 / WPF | P1-P3 WPF 页面完成，P1-P4 Core+CLI 完成，P5 Agent 模板完成
...
- [x] WPF：趋势概览（采集列表 + 指标汇总表，含 Min/Max/Avg/Delta 等）
...
## 下一步优先级
1. ~~**P1 静态相机**~~ -- Core 解析器 + CLI + WPF 页面已完成（HTML 报告后续迭代）
2. ~~**P2 基线差分**~~ -- BaselineService + CLI + WPF 差分页已完成
3. ~~**P3 历史趋势**~~ -- TrendService + CLI + WPF 趋势页已完成（折线图后续迭代）
4. ~~**P4 RenderDoc**~~ -- Core RenderDocService + CLI renderdoc run 已完成（WPF 页后续迭代）
5. ~~**P5 Agent 分析**~~ -- 项目模板（AGENTS.md + Skill）已完成
6. ~~**WPF 页面补齐（P1-P3）**~~ -- 静态相机、差分、趋势 WPF 页面已完成
7. **后续可选** -- RenderDoc WPF 页、静态相机 HTML 报告、趋势折线图