...
### Win64 Launch Runs From The Downloaded Package Directory, Not A Configured Working Directory (Breaking)
- **Breaking, no automatic migration.** `Win64.Executable` in `Config/DefaultGame.ini` is renamed to `Win64.PackageName` and changes meaning from an absolute path to a bare filename (e.g. `MyGame.exe`), and `Win64.WorkingDirectory` is removed outright. `SettingsVersion` stays at 2 — same precedent as the earlier `DeviceSavedRootTemplate` removal: a profile field going away and becoming derived is not a format upgrade. Opening an old project does not fail (the old `Executable`/`WorkingDirectory` keys are simply no longer read — the section falls back to an empty `PackageName`, which fails at launch/capture time with a clear "需要可执行文件名" error rather than silently misbehaving), but existing configs need a manual edit: rename the `Executable` line to `PackageName`, trim its value down to the filename, and delete the `WorkingDirectory` line
- Motivation: a Win64 build already lives on disk because the 安装包 page downloaded it from FTP into `Intermediate/Download/Win64/<version>/`. Asking the user to also maintain an absolute working-directory path pointing at that same data was duplicated state that silently went stale every time the selected package changed
- **`PackageName` is unified across platforms.** `PlatformProfile` gains an abstract `PackageName` member — Android's is the existing app package name (`com.example.game`), Win64's is the executable filename (`MyGame.exe`). Different semantics, same question ("what does this app/process identify as on the device"), so callers that only need the identifier (display, logging) no longer branch by platform. `Win64PlatformProfile`'s primary-constructor parameter is `PackageName` accordingly, and the ini key is `PackageName` too — not just a doc rename, the stored key changes
- `Win64PlatformProfile` also gains `GameRoot`, a runtime-only field the caller (GUI/CLI) injects right before resolving a target — same name as Android's `GameRoot`, but a different lifecycle: Android's is a version-configured template with placeholders, Win64's is a plain absolute path injected per-operation, never written to or read from the ini (`PlatformProfileIni` simply never references it) and not touched by `Validate()`
- `Win64PlatformProfile.Resolve()`: errors if `PackageName` is blank; errors with an actionable message ("请先在「安装包」页下载或选择一个 Win64 构建包") if `GameRoot` is unset; then looks for `PackageName` in that directory's **first level only** — no recursive or cross-directory search — and errors with the full attempted path if it is not there. `GameRootPath` is `GameRoot` itself; `SavedRootPath` is its `Saved` subdirectory, derived through a new shared `PlatformProfile.DeriveSavedRootPath()` helper so Android and Win64 stop each hand-rolling the same split
- `Validate()`'s only new rule: a non-blank `PackageName` must be a bare filename (`Path.GetFileName(PackageName) == PackageName`), catching a leftover absolute path or an accidental subdirectory before it is saved
- The injection point is a single extension method, `ProjectSettings.WithWin64GameRoot(platform, gameRoot)`, called once per operation by whichever caller is about to resolve a target. This exists because **three otherwise-unrelated services all funnel through `ProjectSettings.ResolveTarget`** — `LaunchParameterService` (start/stop, launch target), `CaptureService` (memory/perf capture, `GameRootPath`/`SavedRootPath`), and `UnrealSavedService` (downloading Saved/Logs) — and all three need the package directory on Win64, not just the launch button. Missing one of them would make that one operation silently resolve against a stale or missing directory while the others worked, which is a worse failure mode than a single shared injection point
- Desktop: `ShellViewModel.ResolveWin64PackageDirectory()` picks the 安装包 page's currently-selected package if it happens to be a Win64 selection, otherwise falls back to the most recent local download (natural-sort last), and errors if nothing has been downloaded yet. `ResolveProjectForDevice(device)` wraps that behind a no-op for Android, and every call site that used to pass `_project` directly into `LaunchParameterService`/`CaptureService`/`UnrealSavedService` now passes this resolved copy instead — start, capture preview, capture run, Saved/Log download, and the memreport capture flow all switched together, not just the start button. The launch-parameters view hides the `uecommandline.txt`-specific controls (push/delete/read, that preview panel) for a selected Win64 device via a new `InverseBoolToVisibilityConverter`, since Win64 doesn't read that file — parameters go straight onto the launch command line
- CLI: `app start`, `app console send/run`, `commandline push/delete`, and `capture run` all gain `--package-dir`. `DeviceResolver.ResolveDeviceTargetAsync` enforces it: required when the resolved device is Win64 (CLI has no equivalent of the GUI's "currently selected package" session state), rejected outright when the device is Android rather than silently ignored
- Tests: `Win64PlatformProfile.Resolve()` covering the happy path, missing package directory, and executable-not-found-in-first-level; `Validate()` rejecting a filename with a path separator; ini round-trip confirming `GameRoot` never persists and the `PackageName` key round-trips; `CaptureService` plan creation for Win64 (including the multi-platform-coexist case) rebuilt against real temp directories with an actual file on disk, since `Resolve()` now does a real `File.Exists` check instead of trusting a string

### Console Presets, With Values Read Back From The Running Game
- The 控制台 page grows a grouped preset list above the free-text box. Presets come in three kinds: `Bool` (a checkbox, sends `<cvar> 0|1`), `Value` (a text box, sends `<cvar> <value>`), and `Action` (no input, sends a command such as `stat unit`). Grouping is a display dimension only — the view model holds one flat collection and the view groups it via `CollectionViewSource`, so re-grouping the UI means editing `Group` in the INI, not touching code
- **`Bool` and `Value` presets read their current value back out of the running UE process.** This is the same HTTP endpoint the send path already uses (`PUT /remote/object/call`) with a different function: `GetConsoleVariableBoolValue` / `GetConsoleVariableFloatValue`, parameter `VariableName`, and the current value arrives in the response's top-level `ReturnValue` field. Read-back passes `generateTransaction: false` — it is a read, and does not need an undoable transaction in the engine. Applying a `Bool` or `Value` preset reads back immediately afterwards rather than trusting "the send succeeded, so it took": the engine can clamp a cvar into its legal range, and only a read shows the value that actually landed
- Read-back function names are not driven by `RemoteControlFunctionName`: that setting configures *which function sends a command*, while the two getters are bound to the value type. Sharing the setting would mean that changing the send function name silently breaks reading
- Five layers gain a parallel read path, all gated on the existing `DeviceCapability.SendConsoleCommand` — one channel, so if it can send it can read, and a second capability flag would only ever mirror the first. `IRemoteControlService.QueryConsoleVariableAsync` / `ICommandTransport.QueryConsoleVariableAsync` / `IDeviceService.QueryConsoleVariableAsync` return the raw response body in `ProcessExecutionResult.StandardOutput`; `IConsoleCommandService.QueryVariableAsync` does the single parse into a typed `ConsoleVariableValue`. Android reuses the existing port forward rather than raising a second `adb forward` for reads
- `RemoteControlService` now has one private `CallAsync` carrying the request construction, timeout handling, and status-code error mapping; sending and reading differ only in function name and parameters. Likewise `HttpCommandTransport` maps `RemoteControlException` to a `UKC`-coded `CommandTransportException` in one place for both calls
- Read-back failures do not throw: `ConsoleVariableValue.Failed` carries the reason (channel down with its `UKC` code, empty response, malformed JSON, missing `ReturnValue`, wrong value kind) so a batch refresh reports per-row and keeps going — one unreadable cvar must not void the whole refresh. Nothing is silently substituted with `0`/`false`
- **UE limitation, surfaced rather than papered over:** both getters return `0`/`false` for a cvar that does not exist, indistinguishable from a legitimate `0`/`false`. The UI shows the value it read and never claims "this cvar does not exist"
- Presets are versionable config, mirroring launch presets: `ConsoleCommandPresetDefaults.All` is the built-in floor, and `Config/DefaultGame.ini`'s new `[UnrealKit.ConsoleCommandPresets]` section overrides by name and appends new names. The value format is five fixed fields, `Kind|Group|Cvar|DefaultValue|Command` — fixed positions keep the format hand-writable and let a `Bool` become a `Value` without reordering. An unrecognized `Kind`, a missing `Group`, a `Bool`/`Value` without a `Cvar`, or an `Action` without a `Command` all error on project open instead of falling back, since a silently mis-read preset sends a malformed command when clicked. A name-override keeps the built-in `Description`: the INI has no field for it, and an empty string would wipe the hint from the UI
- `Cvar` and `Command` are separate fields because `Bool`/`Value` need the bare variable name to read back, while `Action` is a whole command line with no readable variable — one shared field leaves "which name do I read?" unanswerable
- `ConsoleCommandPreset.ResolveVariableType()` is a method, not a property: `ProjectSettings` is serialized wholesale into `CaptureManifest.json`, and a property that throws for `Action` presets would fail every capture archive
- New `ParameterizedAsyncDelegateCommand<T>` for the per-row buttons (`CommandParameter="{Binding}"`), kept separate from `AsyncDelegateCommand` rather than changing that type's signature across its many parameterless call sites
- **Contract additions:** `ProjectSettings.ConsoleCommandPresets` (appended after `ConsoleSequences`, existing positional arguments unchanged), the `[UnrealKit.ConsoleCommandPresets]` INI section, and `ConsoleVariableType` in `UnrealKit.Core.CommandChannel` (there rather than in `Console`, which would create a `Devices → Console` namespace cycle). No new diagnostic codes — read-back failures reuse `UKC101`/`UKC102`
- Tests: read-back payload shape and `generateTransaction: false` at the HTTP layer, `ReturnValue` parsing for numbers, `true`/`false`, and `0`/`1` bools, each failure mode's reason text, INI round-trip for all three kinds, default-merge and per-field override, every malformed-config rejection, and the view model's grouping, numeric validation, command enablement, and per-row failure reporting

### ADB Server Starts Once, And Port Forwarding Actually Happens Once
- The adb client starts the daemon by itself whenever a command finds it down, so that cold start landed inside a real command's timeout window and its output got mixed into that command's log. A single explicit `start-server` is now issued before the first command, and skipped from then on. `AdbService.StartServerAsync` previously had no production callers at all — the repeated startup was never ours to begin with
- The flag lives in a new `UnrealKit.Core.Adb.AdbServerLatch`, not in `AdbService`: the GUI builds a fresh `AdbService` per operation, so a per-instance flag would mean one `start-server` per operation — worse than doing nothing. The Desktop factory and the CLI `DeviceResolver` each hold a latch keyed by resolved adb path, since a different `adb.exe` can front a different server version. `AdbService` takes an optional `serverLatch`; omitted, it gets its own
- A failed `start-server` neither throws nor sets the flag: the command right behind it fails with its own exit code and stderr, which says more than a second-hand error, and the next command retries the start. `KillServerAsync` clears the flag so the following command re-ensures the server instead of inheriting a dead one. `StreamLogcatAsync` spawns adb directly rather than through `IProcessRunner`, so it ensures the server too — otherwise daemon startup noise lands in the first lines of the log stream
- **`adb forward` no longer runs on every console command.** `AdbDeviceService` already remembered "this device is forwarded", but `ShellViewModel` rebuilt the device service per operation and threw that record away with it, so each console command really did spawn an extra adb process and mix its output into the command result — exactly what the existing cache was written to prevent. Device services are now cached per `(platform, device id)`
- Two invalidation rules, because the cached record can outlive what it describes: refreshing the device list drops services for devices that are no longer available (an `adb forward` dies with the device, and a reconnect must forward again, or commands hit a tunnel that no longer exists), and opening a project or saving its settings clears the cache entirely (channel port and adb path both come from project config, so stale instances would ignore what was just saved). A device that stays connected across a refresh keeps its forward
- Device enumeration and launch-parameter preview still build throwaway services on purpose — neither forwards a port, and enumeration must not depend on cache state

### Download The Device Saved Directory Or Just Its Logs
- Two new buttons on the WPF 采集归档 page: "下载 Saved" pulls the selected device's whole UE Saved directory, "下载 Log" pulls only its `Logs` subdirectory. Both land the files locally and open the resulting folder. Logs gets its own entry point because troubleshooting usually only needs the logs, while a full Saved can be large (Profiling, screenshots)
- Backed by `UnrealKit.Core.Capture.SavedDownloadService` (`ISavedDownloadService`, `SavedDownloadRequest` / `SavedDownloadPlan` / `SavedDownloadResult`). The two buttons share one landing path, differing only in source directory and wording — the scope is a `SavedDownloadScope` enum (`All` / `Logs`) rather than a caller-supplied subdirectory name, since a free-form path makes "pull the Logs" indistinguishable from "pull a typo", and the latter fails as "the device has no such directory", which reads like a device problem
- Lands in `Saved/DeviceSaved/<Platform>/<yyyyMMdd-HHmmss>-<device id>-<Saved|Logs>/`, never `Content/`. A download carries no `CaptureManifest.json`, so its provenance cannot be reconstructed; putting it under the capture archive would place unattributable data in the authoritative store. The folder name carries timestamp, device id, and scope — dropping any one of them can collide, and a collision surfaces as "the download directory already exists", which reads like a repeated operation rather than two distinct pulls
- Every download goes to a new directory; an existing target directory is an error rather than an overwrite or a merge, so the difference between two pulls is never silently erased. Files stage under `Intermediate/SavedDownloadStaging/` and are moved into place as a unit, so a cancelled or failed pull leaves a staging directory instead of a half-populated result that looks complete. The landing directory holds the requested scope's contents directly — pulling Logs gives you `Game.log`, not `Logs/Game.log`
- A pull that reports success but produces nothing errors out naming the device path and the likely causes, instead of producing an empty directory — "the device has no such directory" and "the pull succeeded but found no data" have to stay distinguishable
- No platform branching: device paths come from `PlatformTarget`, with subdirectories joined via `PlatformTarget.CombineDevicePath` so the separator follows the platform's path style (`Path.Combine` would write backslashes into an Android path on a Windows host), and the transfer delegates to `IDeviceService.PullDirectoryAsync`. A platform that does not support `DeviceCapability.PullDirectory` raises `DeviceCapabilityNotSupportedException` rather than silently doing nothing. Opening the folder afterwards is best-effort: a shell that fails to open is logged as a warning, never reported as a failed download, since the files are already local and the path is displayed
...
- "下载最新" now checks whether the newest build directory already exists under `Intermediate/Download/<Platform>/<subdir>/` before connecting to the FTP server. For an APK download, the directory is considered present only when exactly one `.apk` exists in it or in an immediate child directory; this lets a prior Pak download with the same directory name coexist without suppressing APK retrieval. Directory-mode downloads still skip when the version directory exists. A skipped download reports `DWN007` "already up to date" instead of deleting and re-downloading. The local directory is the re-fetchable cache, so a valid present copy is treated as good unless the user removes it. New `DownloadDiagnosticCodes.AlreadyUpToDate` (`DWN007`); `DownloadResult.Succeeded` remains true for this path, and both the CLI `unrealkit download` and the WPF 安装包 page surface the skip distinctly
